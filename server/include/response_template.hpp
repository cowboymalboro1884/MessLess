...
struct SendActualGlobalConditionResponse : AbstractJSONResponse {
    ...
    messless::GeneralUserInfo general_info;
    int company_id;
    ...
    QJsonDocument to_qjson_document() override {
        ...
        main_data["email"] = QString::fromStdString(user_info.email);
        main_data["password"] = QString::fromStdString(user_info.password);
        main_data["user_role"] = QString::fromStdString(user_info.user_role);
...
struct ProjectCreationIsSuccess final : AbstractJSONResponse {
    ProjectCreationIsSuccess([[maybe_unused]] const std::string &project_name)
        : AbstractJSONResponse(
              response_types[NEW_CONDITION_OF_PROJECTS],
              "success",
              TO_SENDER
