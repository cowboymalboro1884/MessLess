...
Database::Database(const std::string &connection_string,
    ...
    : connection(connection_string.c_str()), crypt(private_salt) {
		std::cout << "всё получилось\n";
		std::cout << connection.dbname() << '\n';
        }
...
PrivateUserInfo DatabaseUser::login_user(Database &db, const std::string &email,
                                         const std::string &password) {
  static std::string personal_salt;
  pqxx::work worker(db.connection);
  try {
    personal_salt =
        worker.query_value<std::string>("SELECT salt FROM users WHERE email='" +
                                        db.shield_string(email) + "';");
  } catch (std::exception &) {
    return {"", "", ""};
  }
...
PrivateUserInfo DatabaseCompany::create_user(
    Database &db, const std::string &email, const std::string &password,
    const std::string &name, const std::string &surname,
    unsigned int company_id, const std::string &user_role) {
