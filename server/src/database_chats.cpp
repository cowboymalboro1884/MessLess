...
unsigned int DatabaseChats::get_chat_id(
    ...
    try {
        ...
        unsigned int chat_id = worker.query_value<int>(
            ...
            "WHERE company_id=" +
            db.shield_string(std::to_string(company_id)) + " AND project_id=" +
...
bool DatabaseChats::send_message(
    ...
    worker.exec(
        "INSERT into messages (chat_id,text_message,sender_id,file_link) "
        "VALUES(" +
        db.shield_string(std::to_string(chat_id)) + ",'" +
        db.shield_string(message) + "'," +
        db.shield_string(std::to_string(user_id)) + ",'" +
        db.shield_string(file_link) + "');"
    );
    worker.commit();
    return true;
}
...
DatabaseChats::get_all_messages(Database &db, unsigned int chat_id) {
    ...
    pqxx::result res = worker.exec(
        "SELECT text_message, file_link, sender_id FROM messages WHERE "
        "chat_id=" +
        db.shield_string(std::to_string(chat_id)) + ";"
    );
    for (auto row : res) {
...
User messless::DatabaseChats::from_private_user_info_to_user(
    messless::Database &db,
    messless::PrivateUserInfo user_info
) {
    pqxx::work worker(db.connection);
    User user;
    unsigned int user_id = worker.query_value<int>(
