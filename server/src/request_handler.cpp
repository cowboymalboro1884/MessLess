...
QJsonDocument RequestHolder::send_message_to_company(const QJsonObject &request
    ...
    json_response_object["name_of_sender"] =
        QString::fromStdString(sender_user.name);
    json_response_object["surname_of_sender"] =
...
QJsonDocument RequestHolder::add_user_to_project(const QJsonObject &request
) const noexcept {
    ...
    messless::DatabaseProject::add_user_in_project(
        *database, email, project_id, user_role
    );
