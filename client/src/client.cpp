...
void Client::start() {
  ...
  connect(m_socketwrapper,
          SIGNAL(got_status_of_registration(
              client::network::PrivateUserInfo,
              std::unordered_map<std::string, std::vector<network::Task>>)),
          this,
          SLOT(got_status_of_registration_slot(
              client::network::PrivateUserInfo,
              std::unordered_map<std::string, std::vector<network::Task>>)));
  connect(m_socketwrapper,
          SIGNAL(got_status_of_authorization(
              client::network::PrivateUserInfo,
              std::unordered_map<std::string, std::vector<network::Task>>)),
          this,
          SLOT(got_status_of_authorization_slot(
              client::network::PrivateUserInfo,
              std::unordered_map<std::string, std::vector<network::Task>>)));
  connect(m_socketwrapper,
          SIGNAL(got_projects_to_update(std::vector<std::string>)), this,
          SLOT(got_projects_to_update_slot(std::vector<std::string>)));
  connect(m_socketwrapper, SIGNAL(somebody_updated_project()), this,
          SLOT(somebody_updated_project_slot()));
  connect(
      m_socketwrapper,
      SIGNAL(got_tasks_to_update(std::vector<client::network::Task>, QString)),
      this,
      SLOT(got_tasks_to_update_slot(std::vector<client::network::Task>,
                                    QString)));
}

// FIXME(to maxim): SIGNALS WITHOUT _slot - SEND DATA, with _slot - PROCESS
...
void Client::got_add_project_data() {
  m_socketwrapper->create_project_request(
      user.email, user.password, user.user_role, m_window->project_name,
      m_window->project_description);
}

void Client::got_project_tasks() {
  m_socketwrapper->get_tasks_request(user.email, user.password, user.user_role,
                                     m_window->project_name);
}

void Client::got_add_task_data() {
  m_socketwrapper->create_task_request(
      user.email, user.password, user.user_role, m_window->task_name,
      m_window->task_description, m_window->task_deadline,
      m_window->project_name);
  got_project_tasks();
}

void Client::got_status_of_registration_slot(
    client::network::PrivateUserInfo new_user,
    std::unordered_map<std::string, std::vector<network::Task>> projects) {
  user = std::move(new_user);
  if (user.status == "success") {
    m_window->ui_Auth.close();
    m_window->ui_Reg.close();
    m_window->show();
    m_window->all_tasks = projects;
  } else {
    //добавить ошибку
  }
}

void Client::got_status_of_authorization_slot(
    network::PrivateUserInfo new_user,
    std::unordered_map<std::string, std::vector<network::Task>> projects) {
  user = std::move(new_user);
  if (user.status == "success") {
    ...
    // TODO:обработать ошибку
    m_socketwrapper->get_projects_request(user.email, user.password,
                                          user.user_role);
  } else {
    //добавить ошибку
  }
...
void Client::got_tasks_to_update_slot(
    std::vector<network::Task> tasks_to_update, QString project_name) {
