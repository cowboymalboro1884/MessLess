...
void Client::start() {
  ...
  connect(
      m_network_manager->m_response_handler,
      SIGNAL(got_user_info(const QString &, const QString &, const QString &,
                           const QString &, const QString &)),
      m_window,
      SLOT(change_user_info(const QString &, const QString &, const QString &,
                            const QString &, const QString &)));
  connect(m_network_manager->m_response_handler,
          SIGNAL(got_project_chats(
              const std::unordered_map<std::string, std::vector<Message>> &)),
          this,
          SLOT(got_project_chats_slot(
              const std::unordered_map<std::string, std::vector<Message>> &)));
}
...
void Client::delete_project(const QString &project) {
  qDebug() << 6;
  m_network_manager->m_query_sender->delete_project(user.email, user.password,
                                                    user.user_role, project);
  qDebug() << 7;
}
...
void Client::got_tasks_to_update_slot(
    const QString &project_name, const std::vector<Task> &tasks_to_update) {
...
void Client::got_project_message_slot(const QString &project_name,
                                      Message message) {
