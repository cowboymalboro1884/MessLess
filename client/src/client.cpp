...
Client::Client(QObject *parent) : m_window(new MainWindow()) {
    ...
    connect(
        m_window, SIGNAL(got_register_data()), this, SLOT(got_register_data())
    );
    connect(
        m_window,
    ...
    connect(
        m_window,
        SIGNAL(
            got_new_task_data(const QString &, const QString &, const QString &)
        ),
        this,
        SLOT(
            got_add_task_data(const QString &, const QString &, const QString &)
        )
    );
    connect(
        m_window,
    ...
    connect(
        m_window,
        SIGNAL(
            add_new_user_to_company(const QString &, const QString &, const QString &, const QString &, const QString &)
        ),
        this,
        SLOT(
            add_user_to_company(const QString &, const QString &, const QString &, const QString &, const QString &)
        )
    );
    connect(
        m_window, SIGNAL(send_message(const QString &)), this,
        SLOT(send_message_to_company(const QString &))
    );
    connect(
        m_window, SIGNAL(delete_project_signal(const QString &)), this,
        SLOT(delete_project(const QString &))
    );
    connect(
        m_window, SIGNAL(delete_user_signal(const QString &)), this,
        SLOT(delete_user(const QString &))
    );
    connect(
        m_window,
        SIGNAL(send_project_message_signal(const QString &, const QString &)),
        this, SLOT(send_project_message(const QString &, const QString &))
    );
};
...
void Client::start() {
    ...
    connect(
        m_network_manager->m_response_handler,
        SIGNAL(got_projects_with_tasks(std::unordered_map<
                                       std::string, std::vector<Task>>)),
        this,
        SLOT(got_projects_to_update_slot(std::unordered_map<
                                         std::string, std::vector<Task>>))
    );
    connect(
        m_network_manager->m_response_handler,
        SIGNAL(got_new_tasks_of_project(const QString &, std::vector<Task>)),
        this, SLOT(got_tasks_to_update_slot(const QString &, std::vector<Task>))
    );
    connect(
        m_network_manager->m_response_handler,
    ...
    connect(
        m_network_manager->m_response_handler,
        SIGNAL(got_company_message(Message)), this,
        SLOT(got_company_message_slot(Message))
    );
    connect(
        m_network_manager->m_response_handler,
        SIGNAL(got_project_message(const QString &, Message)), this,
        SLOT(got_project_message_slot(const QString &, Message))
    );
    connect(
        m_network_manager->m_response_handler,
    ...
    connect(
        m_network_manager->m_response_handler,
        SIGNAL(
            got_user_info(const QString &, const QString &, const QString &, const QString &, const QString &)
        ),
        m_window,
        SLOT(
            change_user_info(const QString &, const QString &, const QString &, const QString &, const QString &)
        )
    );
    connect(m_network_manager->m_response_handler, SIGNAL(got_project_chats(const std::unordered_map<std::string, std::vector<Message>> &)), this, SLOT(got_project_chats_slot(const std::unordered_map<std::string, std::vector<Message>> &)));
}
...
void Client::delete_project(const QString &project) {
    qDebug() << 6;
    m_network_manager->m_query_sender->delete_project(
        user.email, user.password, user.user_role, project
    );
    qDebug() << 7;
}

void Client::got_error_slot(const QString &error) {
    QMessageBox::warning(m_window, "Warning", error);
}

void Client::got_project_chats_slot(const std::unordered_map<std::string, std::vector<Message>> &chats){
    m_window->all_messages = std::move(chats);
}

void Client::send_project_message(
    const QString &project_name,
    const QString &message
) {
    m_network_manager->m_query_sender->send_message_to_project(
        user.email, user.password, user.user_role, message, project_name
    );
}
...
void Client::got_new_task_condition(
    const QString &name,
    const QString &condition
) {
    m_network_manager->m_query_sender->change_task_condition(
        user.email, user.password, user.user_role, name,
        m_window->current_window, condition
    );
}
...
void Client::got_status_of_authorization_or_registration_slot(
    ...
    if (m_window->user_role == "employee") {
...
void Client::add_user_to_company(
    const QString &email,
    const QString &name,
...
void Client::got_project_message_slot(
    const QString &project_name,
    Message message
) {
    m_window->all_messages[project_name.toStdString()].push_back(message);
    m_window->update_project_chat(project_name, message);
}
