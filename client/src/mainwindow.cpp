...
MainWindow::MainWindow(QWidget *parent)
    ...
    update_projects();
    connect(
        &ui_Auth, SIGNAL(login_button_clicked()), this, SLOT(authorizeUser())
    );
    connect(
        &ui_Auth, SIGNAL(register_button_clicked()), this,
        SLOT(registerWindowShow())
    );
    connect(
        &ui_Reg, SIGNAL(pushButtonBack_clicked()), this,
        SLOT(authorizeWindowShow())
    );
    connect(
        &ui_Reg, SIGNAL(register_button_clicked2()), this, SLOT(registerUser())
    );
...
void MainWindow::change_user_info(
    const QString &email,
    const QString &name,
...
void MainWindow::send_project_message(
...
void MainWindow::update_project_chat(
    const QString &project_name,
    Message message
) {
...
void MainWindow::add_new_messages() {
    if (current_window == "main_window_chat") {
        for (size_t index = company_messages.size() - message_counter;
            ...
            QLayout *lay = ui_Main->messages->layout();
            QPushButton *user = new QPushButton(
                company_messages[index].name_of_sender + " " +
...
void MainWindow::add_user_to_project(
    const QString &email,
    const QString &role
) {
...
void MainWindow::add_user_to_company(
    const QString &email,
    const QString &name,
    const QString &surname,
...
void MainWindow::change_task_condition(
    const QString &name,
