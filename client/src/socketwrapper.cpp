...
void SocketWrapper::update() {
    ...
    if (json_data_error.errorString().toInt() == QJsonParseError::NoError) {
        ...
        if (event_type == "got_status_of_authorization") {  ////DONE
            ...
            QString got_password =
                json_data.object().value("password").toString();
            QString got_user_role =
                json_data.object().value("user_role").toString();
            user =
                PrivateUserInfo{status, got_email, got_password, got_user_role};
            emit got_status_of_authorization(user);
            ...
            emit got_status_of_registration(user);
            ...
            while (std::getline(ss1, task, '|')) {
                ...
                tasks_to_update.push_back(Task{
                    QString::fromStdString(task_name),
            ...
            std::string name_of_sender = json_data.object()
                                             .value("name_of_sender")
                                             .toString()
                                             .toStdString();
            ...
            std::string raw_messages =
                json_data.object().value("message").toString().toStdString();
            QString project_name =
                json_data.object().value("message").toString();
...
bool SocketWrapper::add_user_to_project(
    const QString &email_sender,
...
bool SocketWrapper::get_projects_request(
    const QString &email,
...
bool SocketWrapper::get_tasks_request(
    const QString &email,
    const QString &password,
    const QString &user_role,
    const QString &project_name
...
bool SocketWrapper::create_project_request(
    const QString &email,
    const QString &password,
    const QString &user_role,
    const QString &project_name,
...
bool SocketWrapper::validate_user_request(
    const QString &email,
    const QString &password
) {
    QJsonObject jsonquery;
...
bool SocketWrapper::send_message_to_company_request(
    const QString &email,
    const QString &password,
    const QString &user_role,
