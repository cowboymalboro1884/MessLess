...
add_project::add_project(QWidget *parent, MainWindow *main_wind):
   QMainWindow(parent), ui(new Ui::add_project), main_wind(main_wind) {
    ...
    connect(ui->addProject, &QPushButton::clicked, [&]{//мб надо будт изменить на this
...
//            main_wind->get_ui().pro
            ...
            main_wind->centralWidget()->tab; //понять как обращаться нормально к полю и вызывать clear()
