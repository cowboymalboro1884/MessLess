...
ProjectWindow::ProjectWindow(
    ...
    connect(
        main_window, SIGNAL(update_current_tasks()), this,
    ...
    connect(ui->project_chat, &QPushButton::clicked, [&] {
        ...
        ProjectChat *project_chat =
            new ProjectChat(nullptr, this, m_main_window);
        project_chat->show();
    });
}
...
void ProjectWindow::add_user_to_project(
    const QString &name,
    const QString &role
) {
...
void ProjectWindow::update_tasks() {
    ...
    for (const auto &task :
         m_main_window
             ->all_tasks[m_main_window->current_window.toStdString()]) {
        ...
        connect(task_button, &QPushButton::clicked, [&] {
            if (task.condition == "to do") {
                m_main_window->change_task_condition(
