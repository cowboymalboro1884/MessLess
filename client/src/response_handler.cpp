...
extract_projects_with_tasks_from_json(const QJsonObject &request) {
  QJsonArray raw_projects_with_tasks = request["projects_with_tasks"].toArray();
  ...
  for (const auto &project : raw_projects_with_tasks) {
    ...
    for (const auto &task : tasks) {
      ...
      projects_with_tasks[project_name].push_back(
          Task{task_obj["task_name"].toString(),
...
std::vector<Message>
extract_chat_from_json_array(const QJsonArray &message_array) {
  std::vector<Message> chat;

  for (const auto &raw_message : message_array) {
    QJsonObject message_object = raw_message.toObject();
    Message message{message_object["name_of_sender"].toString(),
...
void ResponseHandler::update_projects(const QJsonObject &request) const {
  QJsonArray raw_projects_with_tasks = request["projects_of_company"].toArray();
...
void ResponseHandler::recieved_message_to_company(
    const QJsonObject &request) const {
