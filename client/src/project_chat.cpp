...
ProjectChat::ProjectChat(QWidget *parent, ProjectWindow *project_window,
                         MainWindow *main_window)
    : QWidget(parent), ui(new Ui::ProjectChat),
      m_project_window(project_window), m_main_window(main_window) {
  ui->setupUi(this);
  ...
  connect(ui->send_button, &QPushButton::clicked, [&] {
    if (ui->message_edit->text().isEmpty()) {
      ...
      m_main_window->send_project_message(m_project_window->m_project_name,
                                          ui->message_edit->text());
...
void ProjectChat::update_project_message(Message message) {
  QLayout *lay = ui->messages->layout();
  QPushButton *user =
      new QPushButton(message.name_of_sender + " " + message.surname_of_sender);
  QLabel *user_message = new QLabel(message.message_as_it_is);
  lay->addWidget(user);
  lay->addWidget(user_message);
}
